...
#code from https://itnext.io/build-a-sparse-reward-pysc2-agent-a44e94ba5255
#Modified to use a tensorflow neural net instead of a q-table
...
class QLearningTable:
    def __init__(self, actions, learning_rate=0.0001, reward_decay=0.9, e_greedy=0.9):
        ...
        self.nets = TrainQLearning(self.nets, output_length=1, learning_rate=learning_rate)
        init_op = tf.global_variables_initializer()
        self.sess = tf.Session()
        self.sess.run(init_op)

    def choose_action(self, observation):
        if np.random.uniform() > self.epsilon: # e% of the time choose random action
            if self.epsilon > self.endepsilon:
                self.epsilon = self.epsilon - self.stepDrop
                print(self.epsilon)

            # choose best action
            bestaction = self.actions[0]
            ...
            for action in self.actions:
                ...
                observation_action = np.append(observation,action)
                pred_reward = self.sess.run(
                    [self.nets['predicted_reward']],
                    feed_dict={self.nets['input_to_net']: [observation_action], self.nets['keep_prob']: 1})
            ...
            observation_action = np.append(observation, bestaction)

            bestreward = self.sess.run(
                    [self.nets['predicted_reward']],
                    feed_dict={self.nets['input_to_net']: [observation_action], self.nets['keep_prob']: 1})

            bestreward = bestreward[0][0][0]
    ...
    def learn(self, s, a, r, s_):
        ...
        observation_action = np.append(s,a)#s.append(a)

        q_predict = self.sess.run(
                    [self.nets['predicted_reward']],
                    feed_dict={self.nets['input_to_net']: [observation_action], self.nets['keep_prob']: 1})

        q_predict = q_predict[0][0][0]

        if str(s_) != 'terminal':

            _, max_predict = self.choose_action(s_)

            q_target = r + self.gamma * max_predict
        else:
        ...
        target = q_target #- q_predict

        train = self.sess.run(
            [self.nets['train_step']],
            feed_dict={self.nets['input_to_net']: [observation_action],
...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if obs.last():
            ...
            self.qlearn.learn(self.previous_state, self.previous_action, reward, 'terminal')
        ...
        if self.move_number == 0:
            ...
            rl_action, _ = self.qlearn.choose_action(current_state)
